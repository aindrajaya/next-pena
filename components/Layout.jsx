...
function Header() {
  ...
  return (
    <header
      className={clsx(
        `sticky top-0 ${
          isReferencePage ? 'mt-0' : 'mt-8'
        } z-50 flex flex-none flex-wrap items-center justify-between bg-white px-4 py-4 transition duration-500 sm:px-6 lg:px-8 dark:shadow-none`,
        isScrolled
    ...
    >
      ...
      {isReferencePage || <div className='-my-5'>{/* <Search /> */}</div>}
      <div className='hidden relative md:flex basis-0 justify-end gap-6 sm:gap-8 md:flex-grow'>
        {isReferencePage || <ThemeSelector className='relative z-10' />}
    ...
    <div className='flex w-full flex-row max-h-screen'>
      ...
      <div
        className={`flex flex-col w-full overflow-y-scroll ${
          isReferencePage ? '' : 'px-4 lg:pl-8 lg:pr-0 xl:px-16'
        }`}
      >
        <Header />
